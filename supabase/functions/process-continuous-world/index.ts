...
import { advanceContinuousWorld, type CalibratedPolicyDefinition, type ContinuousPolicyAction, type ContinuousWorldShock, type ContinuousWorldState } from "../../../lib/economics/continuous-world/index.ts";
...
type ShockRow = { id: string; shock_key: string; country_key: string | null; effects: Json; status: "scheduled" | "active" | "expired" | "cancelled"; starts_at: string; ends_at: string };
...
const EFFECT_ORDER = ["growth_pp", "inflation_pp", "unemployment_pp", "debt_gdp_pp", "deficit_gdp_pp", "fx_appreciation_percent", "reserves_import_months", "trade_gdp_pp", "productivity_percent", "emissions_percent", "poverty_pp", "public_support_pp", "stability_points"];
const triple = (value: unknown): [number, number, number] | null => {
  const values = typeof value === "string" ? value.split("/").map(Number) : asArray<unknown>(value).map(Number);
...
function extendedPolicyDefinitions(payload: Json): Record<string, CalibratedPolicyDefinition> {
  const library = asRecord(payload.extended_policy_effect_library);
  return Object.fromEntries(asArray<Json>(library.policies).flatMap((policy) => {
    ...
    const effects = Object.fromEntries(EFFECT_ORDER.flatMap((metric, index) => {
    ...
    if (!id || !allowed || ![lag, ramp, peak, decay, duration].every(Number.isFinite)) return [];
    return [[id, { id, allowed_range: allowed, implementation_lag_days: [lag, lag], ramp_days: [ramp, ramp], peak_days: [peak, peak], decay_half_life_days: [decay, decay], max_duration_days: duration, effects_per_impulse: effects }]];
  }));
}
...
function shockEffects(shock: Json): Json {
  ...
  return {
    inflation_pp: Number((energyPrice * 0.02 + foodPrice * 0.025 + pressure * 0.03).toFixed(4)),
    real_gdp_growth_pp: Number((supply * 0.04).toFixed(4)),
    poverty_pp: Number((burden * 0.12).toFixed(4)),
    stability_points: Number((supply * 0.06 - burden * 0.18 + bank * 0.25).toFixed(4)),
  };
}
...
function automaticShock(payload: Json, worldId: string, processedAt: string): { key: string; durationDays: number; effects: Json } | null {
  const day = processedAt.slice(0, 10);
  const shocks = asArray<Json>(payload.shocks).filter((shock) => typeof shock.id === "string" && Number(shock.annual_probability_prior) > 0);
  const totalDailyProbability = Math.min(0.025, shocks.reduce((sum, shock) => sum + Number(shock.annual_probability_prior) / 365, 0));
  if (!shocks.length || dailyRoll(`${worldId}:${day}`) >= totalDailyProbability) return null;
  const selected = shocks[Math.floor(dailyRoll(`${day}:${worldId}:select`) * shocks.length)] ?? shocks[0];
  const duration = asArray<unknown>(selected.direct_duration_days).map(Number);
  return { key: String(selected.id), durationDays: Math.max(1, Math.round(duration[1] ?? duration[0] ?? 14)), effects: shockEffects(selected) };
}

function stateForTick(row: ClaimedWorld, shocks: ContinuousWorldShock[]): ContinuousWorldState {
...
async function settleContracts(admin: ReturnType<typeof createClient>, worldId: string, processedAt: string) {
  const { data, error } = await admin.from("continuous_world_contracts")
    .select("id,status,terms,starts_at,next_settlement_at")
    .eq("world_id", worldId)
    .in("status", ["scheduled", "in_transit_or_performing", "invoice_due", "late"]);
  if (error) throw new Error(error.message);
  const changes: Json[] = [];
  for (const contract of data ?? []) {
    const terms = asRecord(contract.terms);
    const cycle = Math.max(1, Math.min(180, Number(terms.payment_cycle_days) || 30));
    const due = typeof contract.next_settlement_at === "string" ? Date.parse(contract.next_settlement_at) : 0;
    const starts = typeof contract.starts_at === "string" ? Date.parse(contract.starts_at) : 0;
    const now = Date.parse(processedAt);
    let nextStatus: string | null = null;
    let nextSettlement: string | null = null;
    if (contract.status === "scheduled" && starts <= now) { nextStatus = "in_transit_or_performing"; nextSettlement = new Date(now + cycle * 86_400_000).toISOString(); }
    else if (contract.status === "in_transit_or_performing" && due <= now) { nextStatus = "invoice_due"; nextSettlement = new Date(now + 10 * 86_400_000).toISOString(); }
    else if (contract.status === "invoice_due" && due <= now) { nextStatus = "late"; nextSettlement = new Date(now + 20 * 86_400_000).toISOString(); }
    else if (contract.status === "late" && due <= now) nextStatus = "default_notice";
    if (!nextStatus) continue;
    const { error: updateError } = await admin.from("continuous_world_contracts").update({ status: nextStatus, next_settlement_at: nextSettlement }).eq("id", contract.id);
...
Deno.serve(async (request) => {
  ...
  const { data: packageRows, error: packageError } = await admin.from("calibration_packages").select("package_key,payload").in("package_key", ["policy_effect_library", "shock_library", "final_world_teaching"]).eq("status", "active");
  if (packageError) return reply({ ok: false, message: packageError.message }, 500);
  const packages = Object.fromEntries((packageRows ?? []).map((entry) => [entry.package_key, asRecord(entry.payload)]));
  const definitions = { ...policyDefinitions(packages.policy_effect_library ?? {}), ...extendedPolicyDefinitions(packages.final_world_teaching ?? {}) };
  if (!Object.keys(definitions).length) return reply({ ok: false, message: "No active policy calibration package is available." }, 409);
  const outcomes: Json[] = [];

  for (const row of (claimed ?? []) as ClaimedWorld[]) {
    try {
      const processedAt = new Date().toISOString();
      await admin.from("continuous_world_shocks").update({ status: "active" }).eq("world_id", row.world_id).eq("status", "scheduled").lte("starts_at", processedAt);
      await admin.from("continuous_world_shocks").update({ status: "expired" }).eq("world_id", row.world_id).eq("status", "active").lte("ends_at", processedAt);
      const { data: existingToday, error: existingTodayError } = await admin.from("continuous_world_shocks").select("id").eq("world_id", row.world_id).eq("source", "automatic").gte("starts_at", `${processedAt.slice(0, 10)}T00:00:00.000Z`).limit(1);
      if (existingTodayError) throw new Error(existingTodayError.message);
      const auto = existingToday?.length ? null : automaticShock(packages.shock_library ?? {}, row.world_id, processedAt);
      if (auto) {
        const { error: insertShockError } = await admin.from("continuous_world_shocks").insert({ world_id: row.world_id, shock_key: auto.key, source: "automatic", effects: auto.effects, status: "active", starts_at: processedAt, ends_at: new Date(Date.parse(processedAt) + auto.durationDays * 86_400_000).toISOString() });
        if (insertShockError) throw new Error(insertShockError.message);
      }
      const { data: shockRows, error: shocksError } = await admin.from("continuous_world_shocks").select("id,shock_key,country_key,effects,status,starts_at,ends_at").eq("world_id", row.world_id).eq("status", "active").gt("ends_at", processedAt);
      if (shocksError) throw new Error(shocksError.message);
      const shocks = (shockRows ?? []).map((shock: ShockRow): ContinuousWorldShock => ({ id: shock.id, shockId: shock.shock_key, countryId: shock.country_key, effects: asRecord(shock.effects) as Record<string, number>, status: shock.status, startsAt: shock.starts_at, endsAt: shock.ends_at }));
      ...
      const contractChanges = await settleContracts(admin, row.world_id, processedAt);
      const tick = advanceContinuousWorld(stateForTick(row, shocks), actions, definitions, processedAt);
      ...
      const { error: completeError } = await admin.rpc("complete_continuous_world_tick", {
        ...
        p_effect_summary: [...tick.appliedEffects, ...tick.stateChanges.map((change) => ({ type: "governance_state_change", ...change })), ...contractChanges.map((change) => ({ type: "contract_settlement", ...change }))],
      });
      ...
      if (tick.stateChanges.length) await admin.from("continuous_world_events").insert(tick.stateChanges.map((change) => ({ world_id: row.world_id, country_key: change.countryId, event_type: "state_change", payload: change })));
      if (contractChanges.length) await admin.from("continuous_world_events").insert(contractChanges.map((change) => ({ world_id: row.world_id, event_type: "contract", payload: change })));
      outcomes.push({ worldId: row.world_id, ok: true, stateVersion: tick.state.stateVersion, processedDays: tick.processedDays, shocks: shocks.length + (auto ? 1 : 0), contractChanges: contractChanges.length });
    } catch (error) {
