-- Self-contained repair for projects where legacy helper functions have
-- incompatible signatures. Run this once after either earlier authority
-- migration failed. It neither deletes nor rewrites World state.
...
as $$
  select public.can_manage_continuous_world_country(
      p_world_id, p_country_key, p_user_id
    )
    or exists(
      select 1
...
as $$
declare
  assigned public.continuous_world_country_teams%rowtype;
  current_team_id uuid;
begin
  if auth.uid() is null then raise exception 'Authentication required'; end if;
  current_team_id := public.continuous_world_team_for_user(auth.uid());
  if current_team_id is null then raise exception 'An approved League team is required'; end if;
  if not exists(
    select 1 from public.team_members member
    where member.team_id = current_team_id
      and member.user_id = auth.uid()
      and member.team_role = 'captain'
  )
  and not exists(
    select 1
  ...
  and not public.can_administer_continuous_world(p_world_id, auth.uid()) then
    raise exception 'Only the Team captain, its School Leader, or a Platform Admin may claim a country';
  end if;
  ...
  values(p_world_id, p_country_key, current_team_id, auth.uid()) returning * into assigned;
  ...
  values(
    ...
    p_country_key,
    'notice',
    jsonb_build_object('message', 'A League team claimed this country.', 'team_id', current_team_id),
    auth.uid()
  );
  return assigned;
end;
