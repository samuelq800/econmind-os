...
export const createLiveAuctionItem = (roomId: string, data: { name: string; description: string; startingPrice: number; bidIncrement: number; auctionType: "OPEN" | "SEALED"; openBiddingMode: "ONLINE" | "OFFLINE" | null; assignment: AuctionValueAssignment }) => roomRpc("create_live_auction_item", { p_room_id: roomId, p_name: data.name, p_description: data.description || null, p_starting_price: data.startingPrice, p_bid_increment: data.bidIncrement, p_auction_type: data.auctionType, p_open_bidding_mode: data.openBiddingMode, p_value_assignment: data.assignment });
