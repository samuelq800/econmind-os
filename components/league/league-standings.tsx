...
function ContinuousWorldStanding() {
  return <section className="mt-8 grid gap-5 lg:grid-cols-[1.08fr_.92fr]"><Card className="p-7"><Globe2 className="text-[var(--accent)]" size={24} /><p className="mt-6 text-[10px] font-bold uppercase tracking-[.15em] text-[var(--accent)]">Independent persistent simulation</p><h2 className="mt-2 text-2xl font-bold">Continuous World rankings</h2><p className="mt-3 max-w-2xl text-sm leading-6 text-[var(--ink-muted)]">The current country rankings, risk conditions and role contributions are maintained by the existing Continuous World system. They remain visible in League, but do not create Season 1 points.</p><Link href="/league/world/leaderboard" className="mt-6 inline-flex items-center gap-2 rounded-lg bg-[var(--accent)] px-5 py-3 text-sm font-bold text-white">Open current World ranks <ArrowRight size={15} /></Link></Card><Card className="p-7"><h3 className="text-lg font-bold">Why this is separate</h3><ul className="mt-5 space-y-3 text-sm leading-6 text-[var(--ink-muted)]"><li>• It runs continuously in natural simulated time.</li><li>• Country teams can make policy, trade and contract decisions between visits.</li><li>• Its current ranks measure national conditions and risk, not monthly Challenge points.</li></ul></Card></section>;
}
