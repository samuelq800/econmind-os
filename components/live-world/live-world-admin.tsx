...
export function LiveWorldAdmin() {
  const { user, role, platformRole, loading, openAuth } = useAuth();
  const liveSessionHost = canHostLiveSession(role, platformRole);
  const [name, setName] = useState("Live World Session");
  const [duration, setDuration] = useState(10);
  const [participantCapacity, setParticipantCapacity] = useState(20);
  ...
  async function create(event: React.FormEvent) {
    ...
    try { const next = await createLiveWorldRoom(name, duration * 60, participantCapacity); setCreated(next); await refresh(); }
  ...
  if (!user) return <main className="mx-auto max-w-6xl px-5 py-12"><Card className="max-w-xl p-7"><ShieldCheck className="text-[var(--accent)]" size={22} /><h1 className="mt-4 text-2xl font-bold">Live session host access</h1><p className="mt-2 text-sm leading-6 text-[var(--ink-muted)]">Sign in with a Teacher, School Leader, or Platform Admin account to create and control Live World rooms.</p><Button className="mt-5" onClick={() => openAuth("sign-in")}>Sign in</Button></Card></main>;
  if (!liveSessionHost) return <main className="mx-auto max-w-6xl px-5 py-12"><Card className="max-w-xl p-7"><ShieldCheck className="text-[var(--red)]" size={22} /><h1 className="mt-4 text-2xl font-bold">Live session host access required</h1><p className="mt-2 text-sm leading-6 text-[var(--ink-muted)]">Teachers, School Leaders, and Platform Admins can create a short Live World event.</p></Card></main>;

  return <main className="mx-auto min-h-screen max-w-6xl px-5 py-12 sm:px-8"><header><p className="text-[10px] font-extrabold uppercase tracking-[.2em] text-[var(--accent)]">Live session hosting</p><h1 className="mt-3 flex items-center gap-3 text-4xl font-bold tracking-[-.05em]"><Gamepad2 className="text-[var(--accent)]" /> Live World Rooms</h1><p className="mt-3 max-w-3xl text-sm leading-6 text-[var(--ink-muted)]">Create a self-contained four-country, five-office economic simulation for up to 20 players. Each room gets a random standalone link; visitors open it first, then enter a player or administrator code without linking an EconMind account.</p></header><div className="mt-8 grid items-start gap-6 lg:grid-cols-[.85fr_1.15fr]"><Card className="p-6"><h2 className="text-xl font-bold">Create a room</h2><form className="mt-5 grid gap-5" onSubmit={(event) => void create(event)}><label className="grid gap-2 text-xs font-bold">Room name<input maxLength={120} value={name} onChange={(event) => setName(event.target.value)} className="h-10 rounded-lg border border-[var(--line)] bg-[var(--canvas)] px-3 text-sm" required /></label><label className="grid gap-2 text-xs font-bold">Duration<select value={duration} onChange={(event) => setDuration(Number(event.target.value))} className="h-10 rounded-lg border border-[var(--line)] bg-[var(--canvas)] px-3 text-sm">{[5, 10, 15, 20, 30, 45, 60].map((minutes) => <option key={minutes} value={minutes}>{minutes} minutes</option>)}</select></label><label className="grid gap-2 text-xs font-bold">Player capacity<input type="number" min={1} max={20} value={participantCapacity} onChange={(event) => setParticipantCapacity(Math.min(20, Math.max(1, Number(event.target.value) || 1)))} className="h-10 rounded-lg border border-[var(--line)] bg-[var(--canvas)] px-3 text-sm" /><span className="text-[11px] font-normal leading-4 text-[var(--ink-muted)]">There are 20 cabinet offices in total. You can also adjust this limit before the room starts.</span></label>{error && <p role="alert" className="rounded-lg bg-[var(--red-soft)] p-3 text-xs text-[var(--red)]">{error}</p>}<Button type="submit" disabled={busy}>{busy && <LoaderCircle className="animate-spin" size={15} />}{busy ? "Creating…" : "Create Live World room"}</Button></form>{created && <section className="mt-6 rounded-xl border border-[var(--accent)] bg-[var(--accent-soft)] p-4"><p className="text-xs font-bold text-[var(--accent)]">Share the random room link, then send the appropriate invitation code separately. The codes are not shown again.</p><Code label="Player code" value={created.playerCode} /><Code label="Administrator code" value={created.adminCode} /><div className="mt-4 flex flex-wrap gap-2"><Link href={liveWorldRoomPath(created.room.id)} target="_blank" className="inline-flex h-9 items-center rounded-lg bg-[var(--accent)] px-3 text-xs font-bold text-white transition-transform active:scale-[.96]">Open room</Link><CopyAction label="Copy room link" value={() => liveWorldRoomUrl(window.location.origin, created.room.id)} /></div></section>}</Card><section><div className="flex items-center justify-between"><h2 className="text-xl font-bold">Existing rooms</h2><Button size="sm" variant="secondary" disabled={refreshing} onClick={() => void refresh()}>{refreshing && <LoaderCircle className="animate-spin" size={13} />}{refreshing ? "Refreshing…" : "Refresh"}</Button></div><div className="mt-4 grid gap-3">{rooms.map((room) => <Card key={room.id} className="p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-[.12em] text-[var(--accent)]">{room.status}</p><h3 className="mt-1 text-lg font-bold">{room.name}</h3><p className="mt-1 text-xs text-[var(--ink-muted)]">{Math.round(room.duration_seconds / 60)} minutes · {room.participant_count} people entered · {room.participant_capacity} player places</p></div><Link href={liveWorldRoomPath(room.id)} target="_blank" className="rounded-lg border border-[var(--line)] px-3 py-2 text-xs font-bold text-[var(--accent)] transition-[background-color,transform] hover:bg-[var(--accent-soft)] active:scale-[.96]">Open control room</Link></div></Card>)}{!rooms.length && <Card className="p-6 text-sm text-[var(--ink-muted)]">No Live World rooms have been created yet.</Card>}</div></section></div></main>;
}
