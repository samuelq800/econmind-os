...
function LiveWorldEntry({ roomId, error, onJoined }: { roomId: string; error: string; onJoined: () => Promise<void> }) {
  ...
  return <div className="grid min-h-[75vh] items-center lg:grid-cols-[1.1fr_.9fr] lg:gap-14"><section><p className="text-[11px] font-extrabold uppercase tracking-[.25em] text-[#62cbb0]">EconMind Live World</p><h1 className="mt-5 max-w-3xl text-5xl font-black tracking-[-.065em] sm:text-7xl">A live economic world.</h1><p className="mt-6 max-w-xl text-base leading-7 text-[#a7bbb1]">Enter the event code to take a cabinet seat, supervise the room, or follow the shared scoreboard. This event view is intentionally separate from the rest of EconMind.</p><div className="mt-10 grid gap-3 sm:grid-cols-3">{["4 countries", "12 cabinet seats", "One shared clock"].map((item) => <div key={item} className="rounded-xl border border-[#28443a] bg-[#10231c] px-4 py-3 text-xs font-bold text-[#c8d7d0]">{item}</div>)}</div></section><Card className="border-[#345c4c] bg-[#10231c] p-6 shadow-2xl"><p className="text-[10px] font-extrabold uppercase tracking-[.18em] text-[#62cbb0]">Room access</p><h2 className="mt-2 text-2xl font-bold">Join the event</h2><p className="mt-2 text-sm leading-6 text-[#a7bbb1]">Use a player or administrator invitation code. No EconMind account is required.</p><form className="mt-6 grid gap-4" onSubmit={(event) => void submit(event)}><label className="grid gap-2 text-xs font-bold">Display name<input autoComplete="nickname" maxLength={48} value={displayName} onChange={(event) => setDisplayName(event.target.value)} className="h-11 rounded-lg border border-[#365c4d] bg-[#07120f] px-3 text-sm outline-none focus:border-[#62cbb0]" placeholder="Enter your name" required /></label><label className="grid gap-2 text-xs font-bold">Invitation code<input autoCapitalize="characters" value={code} onChange={(event) => setCode(event.target.value.toUpperCase())} className="h-11 rounded-lg border border-[#365c4d] bg-[#07120f] px-3 font-mono text-sm uppercase tracking-wider outline-none focus:border-[#62cbb0]" placeholder="PLAY-XXXXXXXXXX" required /></label>{message && <p role="alert" className="rounded-lg bg-[#542c31] px-3 py-2 text-xs text-[#ffb8bb]">{message}</p>}<Button type="submit" disabled={busy}>{busy && <LoaderCircle size={15} className="animate-spin" />}{busy ? "Entering…" : "Enter Live World"}</Button></form></Card></div>;
}
