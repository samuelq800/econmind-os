...
const numericControls: Record<NewLabKey, ModelParameter[]> = {
  ...
  "phillips-curve": [
    ...
    { id: "demandPressure", label: "Demand pressure", symbol: "d", description: "An unemployment-equivalent demand effect: positive pressure moves the point left along the SRPC.", min: -5, max: 8, step: 0.5, defaultValue: DEFAULT_PHILLIPS.demandPressure },
  ],
  "solow-growth": [
...
function buildDefinition(model: NewLabKey, values: Values): Definition {
  ...
  if (model === "prisoners-dilemma") {
    ...
    return {
      ...
      equations: [{ label: "Best response", expression: "BR_A(b) = arg max_a u_A(a, b); BR_B(a) = arg max_b u_B(a, b)" }, { label: "Dominance", expression: "Strictly dominant: A = " + (out.aStrictDominant.join(", ") || "none") + ", B = " + (out.bStrictDominant.join(", ") || "none") + "; dominated: A = " + (out.aDominated.join(", ") || "none") + ", B = " + (out.bDominated.join(", ") || "none") }, { label: "Nash condition", expression: "(a*, b*) is Nash when a* ∈ BR_A(b*) and b* ∈ BR_B(a*)" }, { label: "Current equilibria", expression: "Pure Nash cells = " + (out.nash.join(", ") || "∅") }, { label: "Pareto test", expression: "A cell is Pareto efficient when no other cell weakly improves both payoffs and strictly improves one." }],
  ...
  if (model === "phillips-curve") {
    ...
    return {
      ...
      mechanism: chain("Inflation expectations, unemployment, or shocks change.", p.supplyShock !== 0 ? "A supply shock shifts the SRPC." : p.expectedInflation !== DEFAULT_PHILLIPS.expectedInflation ? "Expected inflation shifts the SRPC." : "Demand pressure and unemployment move the point along the current SRPC.", "Inflation pressure changes relative to expected inflation.", "Wage and price setting are represented only by the stylised equation.", "Inflation is " + out.inflation + "% at effective unemployment " + out.effectiveUnemployment + "%.", "In the long run, the model references unemployment " + p.naturalUnemployment + "% rather than a permanent trade-off."),
      equations: [{ label: "Short-run Phillips curve", expression: "π = πe − α(u_eff − un) + v; u_eff = u − d" }, { label: "Current substitution", expression: "π = " + p.expectedInflation + " − " + p.sensitivity + "((" + p.unemployment + " − " + p.demandPressure + ") − " + p.naturalUnemployment + ") + " + p.supplyShock }, { label: "Inflation", expression: "π = " + out.inflation + "%" }, { label: "Long-run reference", expression: "LRPC: u = un = " + p.naturalUnemployment + "%" }],
