...
import type { ResearchPaper, ResearchPaperStatus, ResearchPaperSubmission } from "@/lib/research/types";
...
const blankSubmission: ResearchPaperSubmission = {
  ...
  coauthors_text: null, ai_linking_consent: true, visibility: "public",
};

const statusLabel: Record<ResearchPaperStatus, string> = {
  draft: "Draft", pending_review: "Pending review", published: "Published", unpublished: "Unpublished", revision_requested: "Revision requested", rejected: "Rejected",
};
...
function paperToSubmission(paper: ResearchPaper): ResearchPaperSubmission {
  return {
    ...
    coauthors_text: paper.coauthors_text, ai_linking_consent: paper.ai_linking_consent, visibility: "public",
  };
}
...
export function StatusBadge({ status }: { status: ResearchPaperStatus }) {
  const tones: Record<ResearchPaperStatus, string> = {
    ...
    unpublished: "bg-[var(--red-soft)] text-[var(--red)]",
...
export function PaperCard({ paper, featured = false }: { paper: ResearchPaper; featured?: boolean }) {
  return <article className={`group flex h-full flex-col rounded-xl border bg-[var(--surface)] p-5 shadow-[var(--shadow)] transition-colors hover:border-[var(--accent)] ${featured ? "border-[var(--accent)]" : "border-[var(--line)]"}`}>
    <div className="flex items-start justify-between gap-3">
      <div className="flex flex-wrap gap-2"><Badge>{paper.subject}</Badge>{paper.is_professor_upload && <Badge className="border-violet-300 bg-violet-50 text-violet-900 dark:border-violet-800 dark:bg-violet-950/40 dark:text-violet-100">Professor</Badge>}{featured && <Badge className="border-[var(--accent)] bg-[var(--accent-soft)] text-[var(--accent)]"><Award size={11} /> Featured</Badge>}</div>
...
export function ResearchPaperDetail() {
  ...
  const metadata = [["Subject", paper.subject], ["Competition", paper.competition_text], ["Year", paper.competition_year ? String(paper.competition_year) : null], ["Award", paper.award_text], ["Co-authors", paper.coauthors_text], ["Keywords", paper.keywords], ["Author account", paper.is_professor_upload ? "Professor upload" : null], ["AI linking", paper.ai_linking_consent ? "Permitted by the author" : "Prohibited by the author"]].filter((entry): entry is [string, string] => Boolean(entry[1]));
  return <main className="mx-auto min-h-screen max-w-6xl px-5 py-10 sm:px-8 lg:py-14">
    ...
    <header className="mt-7 border-b border-[var(--line)] pb-9">
      <div className="flex flex-wrap items-start justify-between gap-5"><div><div className="flex flex-wrap gap-2"><Badge>{paper.subject}</Badge>{paper.is_professor_upload && <Badge className="border-violet-300 bg-violet-50 text-violet-900 dark:border-violet-800 dark:bg-violet-950/40 dark:text-violet-100">Professor upload</Badge>}</div><h1 className="mt-5 max-w-4xl text-3xl font-bold tracking-[-.055em] sm:text-5xl">{paper.title}</h1><p className="mt-5 text-base font-semibold">{paper.author_display_name}</p><p className="mt-1 text-sm text-[var(--ink-muted)]">{paper.school_display_name}</p></div>{paper.award_verified && <span className="inline-flex items-center gap-2 rounded-lg bg-[var(--accent-soft)] px-3 py-2 text-xs font-bold text-[var(--accent)]"><BadgeCheck size={15} />Award verified</span>}</div>
...
export function ResearchSubmissionForm() {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (editing && paperId) {
        const updated = await updateResearchPaper(paperId, form);
        setMessage(updated.status === "unpublished" ? "Research details saved. This paper remains unavailable until an administrator restores it." : "Research details saved and published.");
      } else {
        ...
        setMessage("Research published and available to everyone.");
        setFile(null);
      }
  ...
  if (loading) return <main className="grid min-h-[65vh] place-items-center"><LoaderCircle className="animate-spin text-[var(--accent)]" /></main>;
  return <main className="mx-auto min-h-screen max-w-4xl px-5 py-10 sm:px-8 lg:py-14">
    <Link href="/learn/research" className="text-xs font-bold text-[var(--accent)]">← Research Library</Link>
    <header className="mt-7 border-b border-[var(--line)] pb-8">
      <p className="text-[10px] font-extrabold uppercase tracking-[.18em] text-[var(--accent)]">Member submission</p>
      <h1 className="mt-3 text-4xl font-bold tracking-[-.055em]">{editing ? "Edit Research" : "Submit Research"}</h1>
      <p className="mt-3 max-w-2xl text-sm leading-6 text-[var(--ink-muted)]">Required: title, author, school, subject, and a PDF. Everything else is optional. Completed uploads are immediately public and readable by everyone.</p>
      {role === "professor" && <p className="mt-4 inline-flex rounded-full border border-violet-300 bg-violet-50 px-3 py-1.5 text-xs font-bold text-violet-900 dark:border-violet-800 dark:bg-violet-950/40 dark:text-violet-100">Professor account — this upload will carry a Professor badge.</p>}
    </header>
    <form onSubmit={(event) => void submit(event)} className="mt-8 space-y-8">
      <Card className="p-5 sm:p-7"><h2 className="text-xl font-bold">Required information</h2><div className="mt-6 grid gap-5 sm:grid-cols-2"><Field label="Title *" wide><input required value={form.title} maxLength={240} onChange={(event) => set("title", event.target.value)} /></Field><Field label="Author *"><input required value={form.author_display_name} maxLength={160} onChange={(event) => set("author_display_name", event.target.value)} /></Field><Field label="School *"><input required value={form.school_display_name} maxLength={240} onChange={(event) => set("school_display_name", event.target.value)} placeholder="Your school or institution" /></Field><Field label="Subject *"><input required value={form.subject} maxLength={160} onChange={(event) => set("subject", event.target.value)} placeholder="e.g. Labour Economics" /></Field>{!editing && <Field label="Paper / File *" wide><input required accept="application/pdf,.pdf" type="file" onChange={(event) => setFile(event.target.files?.[0] ?? null)} /><span className="mt-2 block text-[11px] font-normal text-[var(--ink-muted)]">PDF only, up to 25 MB.</span></Field>}</div></Card>
      <Card className="p-5 sm:p-7"><h2 className="text-xl font-bold">Optional information</h2><div className="mt-6 grid gap-5 sm:grid-cols-2"><Field label="Competition"><input value={optional(form.competition_text)} maxLength={240} onChange={(event) => set("competition_text", event.target.value || null)} placeholder="e.g. John Locke Essay Competition" /></Field><Field label="Year"><input type="number" min="1900" max="2100" value={form.competition_year ?? ""} onChange={(event) => set("competition_year", event.target.value ? Number(event.target.value) : null)} /></Field><Field label="Award / Result"><input value={optional(form.award_text)} maxLength={240} onChange={(event) => set("award_text", event.target.value || null)} placeholder="e.g. Finalist" /></Field><Field label="Keywords"><input value={optional(form.keywords)} maxLength={600} onChange={(event) => set("keywords", event.target.value || null)} placeholder="Comma-separated keywords" /></Field><Field label="Co-authors" wide><input value={optional(form.coauthors_text)} maxLength={1000} onChange={(event) => set("coauthors_text", event.target.value || null)} placeholder="Optional co-author names" /></Field><Field label="Abstract" wide><textarea rows={5} value={optional(form.abstract)} maxLength={12000} onChange={(event) => set("abstract", event.target.value || null)} placeholder="Optional abstract" /></Field><Field label="Description / Note" wide><textarea rows={3} value={optional(form.description)} maxLength={4000} onChange={(event) => set("description", event.target.value || null)} placeholder="Optional note for readers" /></Field></div><label className="mt-6 flex items-start gap-3 rounded-xl border border-[var(--line)] bg-[var(--surface-subtle)] p-4 text-sm leading-6"><input checked={!form.ai_linking_consent} onChange={(event) => set("ai_linking_consent", !event.target.checked)} type="checkbox" className="mt-1 size-4 accent-[var(--accent)]" /><span><strong className="block text-[var(--ink)]">Prohibit AI linking</strong><span className="mt-1 block text-[var(--ink-muted)]">AI linking is enabled by default. Select this only if this paper must not be connected to future EconMind AI features.</span></span></label></Card>
      <Card className="p-5 sm:p-7"><h2 className="text-xl font-bold">Publication permission</h2><label className="mt-5 flex items-start gap-3 text-sm leading-6 text-[var(--ink-muted)]"><input required checked={permission} onChange={(event) => setPermission(event.target.checked)} type="checkbox" className="mt-1 size-4 accent-[var(--accent)]" /><span>I confirm that I am an author of this work, or that I have permission from the relevant author(s) to publish it on EconMind. I understand that I retain ownership of my work and grant EconMind a non-exclusive right to display and distribute this submitted version through the Research Library.</span></label><p className="mt-4 text-xs font-semibold text-[var(--ink-muted)]">Copyright remains with the author(s).</p></Card>
      {error && <p role="alert" className="rounded-xl bg-[var(--red-soft)] p-4 text-sm text-[var(--red)]">{error}</p>}
      {message && <p className="flex items-center gap-2 rounded-xl bg-[var(--accent-soft)] p-4 text-sm font-semibold text-[var(--accent)]"><CheckCircle2 size={17} />{message}</p>}
      <div className="flex justify-end"><Button type="submit" disabled={busy}>{busy ? <LoaderCircle className="animate-spin" size={15} /> : <Send size={15} />}{busy ? "Publishing…" : editing ? "Save changes" : "Publish Research"}</Button></div>
    </form>
  </main>;
}
...
export function ResearchAdmin() {
  ...
  const [status, setStatus] = useState<Extract<ResearchPaperStatus, "published" | "unpublished">>("published");
  ...
  useEffect(() => { if (selected) queueMicrotask(() => { setNote(selected.admin_review_note ?? ""); setStatus(selected.status === "unpublished" ? "unpublished" : "published"); setVerified(selected.award_verified); setFeatured(selected.featured); setDraft(paperToSubmission(selected)); }); }, [selected]);
  const review = async () => { if (!selected) return; setBusy(true); setError(""); try { const next = await reviewResearchPaper({ paperId: selected.id, status, internalNote: note, awardVerified: verified, featured }); setPapers((current) => current.map((paper) => paper.id === next.id ? next : paper)); setMessage(status === "published" ? "Paper restored to the public library." : "Paper taken down from the public library."); } catch (caught) { setError(caught instanceof Error ? caught.message : "Could not update publication access."); } finally { setBusy(false); } };
  ...
  if (!worldSupervisor) return <AccessGate title="Platform administrator access required" detail="Your account does not have access to Research Library management." />;
  return <main className="mx-auto min-h-screen max-w-7xl px-5 py-10 sm:px-8 lg:py-14"><header className="border-b border-[var(--line)] pb-8"><p className="text-[10px] font-extrabold uppercase tracking-[.18em] text-[var(--accent)]">Platform administration</p><h1 className="mt-3 text-4xl font-bold tracking-[-.055em]">Research Library Management</h1><p className="mt-3 text-sm leading-6 text-[var(--ink-muted)]">Every completed upload is published for everyone immediately. Administrators can take down a paper or restore it here.</p></header>{error && <p role="alert" className="mt-6 rounded-xl bg-[var(--red-soft)] p-4 text-sm text-[var(--red)]">{error}</p>}{message && <p className="mt-6 rounded-xl bg-[var(--accent-soft)] p-4 text-sm font-semibold text-[var(--accent)]">{message}</p>}<section className="mt-8 grid gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(340px,.9fr)]"><div className="space-y-3">{papers.map((paper) => <button type="button" key={paper.id} onClick={() => setSelectedId(paper.id)} className={`w-full rounded-xl border p-5 text-left ${selectedId === paper.id ? "border-[var(--accent)] bg-[var(--accent-soft)]" : "border-[var(--line)] bg-[var(--surface)] hover:bg-[var(--surface-subtle)]"}`}><div className="flex flex-wrap items-center justify-between gap-3"><StatusBadge status={paper.status} /><span className="text-[10px] text-[var(--ink-faint)]">Updated {readableDate(paper.updated_at)}</span></div><h2 className="mt-3 text-base font-bold">{paper.title}</h2><p className="mt-1 text-xs text-[var(--ink-muted)]">{paper.author_display_name} · {paper.school_display_name}</p></button>)}{papers.length === 0 && <Card className="p-10 text-center text-sm text-[var(--ink-muted)]">No research has been uploaded yet.</Card>}</div>{selected && <aside className="space-y-5"><Card className="p-5"><div className="flex items-start justify-between gap-3"><div><StatusBadge status={selected.status} /><h2 className="mt-3 text-xl font-bold">{selected.title}</h2><p className="mt-1 text-sm text-[var(--ink-muted)]">{selected.author_display_name} · {selected.school_display_name}</p></div><Link href={`/learn/research/paper?paper=${encodeURIComponent(selected.id)}`}><Button size="sm" variant="secondary">Preview</Button></Link></div><div className="mt-6 grid gap-4"><Field label="Publication status"><select value={status} onChange={(event) => setStatus(event.target.value as Extract<ResearchPaperStatus, "published" | "unpublished">)}><option value="published">Published — everyone can read</option><option value="unpublished">Unpublished — taken down</option></select></Field><Field label="Administrative note"><textarea rows={4} value={note} onChange={(event) => setNote(event.target.value)} placeholder="Optional internal note about this publication action." /></Field><label className="flex items-center gap-2 text-xs font-bold"><input checked={verified} onChange={(event) => setVerified(event.target.checked)} type="checkbox" className="size-4 accent-[var(--accent)]" /> Award verified</label><label className="flex items-center gap-2 text-xs font-bold"><input checked={featured} disabled={status !== "published"} onChange={(event) => setFeatured(event.target.checked)} type="checkbox" className="size-4 accent-[var(--accent)]" /> Featured research</label><Button disabled={busy} onClick={() => void review()}>{busy && <LoaderCircle className="animate-spin" size={14} />}{status === "published" ? "Save and publish" : "Take down paper"}</Button></div></Card><Card className="p-5"><h3 className="text-lg font-bold">Edit metadata</h3><p className="mt-2 text-xs leading-5 text-[var(--ink-muted)]">All Research Library papers are public to everyone.</p><div className="mt-5 grid gap-4"><Field label="Title"><input value={draft.title} onChange={(event) => setDraft({ ...draft, title: event.target.value })} /></Field><Field label="Author"><input value={draft.author_display_name} onChange={(event) => setDraft({ ...draft, author_display_name: event.target.value })} /></Field><Field label="School"><input value={draft.school_display_name} onChange={(event) => setDraft({ ...draft, school_display_name: event.target.value })} /></Field><Field label="Subject"><input value={draft.subject} onChange={(event) => setDraft({ ...draft, subject: event.target.value })} /></Field><Field label="Competition"><input value={optional(draft.competition_text)} onChange={(event) => setDraft({ ...draft, competition_text: event.target.value || null })} /></Field><Field label="Award"><input value={optional(draft.award_text)} onChange={(event) => setDraft({ ...draft, award_text: event.target.value || null })} /></Field><Button variant="secondary" disabled={busy} onClick={() => void saveMetadata()}>Save metadata</Button></div></Card></aside>}</section></main>;
}
