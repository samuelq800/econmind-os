    ...
    <div className="mt-8 grid gap-6 xl:grid-cols-[270px_minmax(0,1fr)]">
      ...
      <section className="min-w-0">
        <div className="rounded-xl border border-[var(--line)] bg-[var(--surface)] p-5 sm:p-7">
          ...
          <ModelFlowDiagram title="From the stated shock to the checkable outcome" stages={[
            { label: "Situation", title: challenge.territory, description: "The fixed initial state in this challenge." },
            { label: "Model", title: selectedModels.length ? selectedModels.map(human).join(" + ") : "Choose a mechanism", description: selectedModels.length ? "Selected mechanisms are highlighted in the answer check." : "Select one or more prescribed models below.", active: selectedModels.length > 0 },
            { label: "Intervention", title: Object.keys(challenge.adjustable).map(human).join(" · "), description: "Only these stated policy controls can change." },
            { label: "Outcome", title: Object.keys(challenge.goal).map(human).join(" · "), description: "The preset evaluates the disclosed success conditions." },
          ]} />
