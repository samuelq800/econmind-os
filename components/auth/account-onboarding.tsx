...
export function AccountOnboarding() {
  ...
  if (!user || roleLoading || checkedUserId !== userId || completedUserId === userId) return null;
  ...
  return <div className="fixed inset-0 z-[110] overflow-y-auto bg-black/50 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="account-setup-title">
    <div className="mx-auto my-6 w-full max-w-2xl rounded-2xl border border-[var(--line)] bg-[var(--surface)] p-5 shadow-2xl sm:my-12 sm:p-7">
      ...
      {step === "school" && <div className="mt-7"><div className="flex items-center justify-between gap-3"><h3 className="text-base font-bold">Choose an approved school</h3><button type="button" onClick={() => setStep("choose")} className="text-xs font-bold text-[var(--accent)]">Back</button></div><p className="mt-2 text-xs leading-5 text-[var(--ink-muted)]">After choosing, use your team’s invite code to become a League participant. A school selection alone cannot claim a country.</p><div className="mt-4 max-h-64 space-y-2 overflow-y-auto pr-1">{schools.map((school) => <label key={school.id} className={`flex cursor-pointer items-center gap-3 rounded-lg border p-3 ${selectedSchoolId === school.id ? "border-[var(--accent)] bg-[var(--accent-soft)]" : "border-[var(--line)] bg-[var(--canvas)]"}`}><input type="radio" name="school" value={school.id} checked={selectedSchoolId === school.id} onChange={() => setSelectedSchoolId(school.id)} className="accent-[var(--accent)]" /><span className="min-w-0"><span className="block text-sm font-bold">{school.name}</span><span className="mt-0.5 flex items-center gap-1 text-[11px] text-[var(--ink-muted)]">{school.city && <><MapPin size={11} />{school.city}</>}{school.city && school.club_name && <span>·</span>}{school.club_name}</span></span></label>)}{schools.length === 0 && <p className="rounded-lg bg-[var(--surface-subtle)] p-4 text-sm text-[var(--ink-muted)]">No approved schools are listed yet. You can submit a school instead or continue as a visitor.</p>}</div><Button className="mt-5 w-full" disabled={!selectedSchoolId || busy} onClick={() => void finish("school")}>{busy && <LoaderCircle className="animate-spin" size={15} />}Continue with selected school</Button></div>}

      {step === "create_school" && <div className="mt-7"><div className="flex items-center justify-between gap-3"><h3 className="text-base font-bold">Submit a school for approval</h3><button type="button" onClick={() => setStep("choose")} className="text-xs font-bold text-[var(--accent)]">Back</button></div><p className="mt-2 text-xs leading-5 text-[var(--ink-muted)]">A teacher administrator reviews this request before the school can create teams or participate in the world.</p><label className="mt-5 block text-xs font-bold">School name<input autoFocus required maxLength={160} value={schoolName} onChange={(event) => setSchoolName(event.target.value)} className="mt-2 h-11 w-full rounded-lg border border-[var(--line-strong)] bg-[var(--canvas)] px-3 text-sm outline-none focus:border-[var(--accent)]" placeholder="Your school" /></label><label className="mt-4 block text-xs font-bold">Economics club name <span className="font-normal text-[var(--ink-faint)]">(optional)</span><input maxLength={160} value={clubName} onChange={(event) => setClubName(event.target.value)} className="mt-2 h-11 w-full rounded-lg border border-[var(--line-strong)] bg-[var(--canvas)] px-3 text-sm outline-none focus:border-[var(--accent)]" placeholder="Economics Club" /></label><Button className="mt-5 w-full" disabled={schoolName.trim().length < 2 || busy} onClick={() => void finish("create_school")}>{busy && <LoaderCircle className="animate-spin" size={15} />}Submit for approval</Button></div>}
