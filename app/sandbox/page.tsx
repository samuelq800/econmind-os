  ...
  return (
    <main className="min-h-screen">
      ...
      <div className="mx-auto grid max-w-[1600px] gap-5 p-4 sm:p-6 xl:grid-cols-[320px_minmax(0,1fr)_340px] xl:items-start">
        <aside className="scroll-slim rounded-xl border border-[var(--line)] bg-[var(--surface)] p-4 xl:sticky xl:top-20 xl:max-h-[calc(100vh-6rem)] xl:overflow-y-auto">
...
function ContributionBreakdown({ result }: { result: SandboxResult }) {
  ...
  return <ChartPanel title="Transparent calculation breakdown" subtitle="Standardized teaching coefficients — direct and interaction effects reconcile to the final result, including a labelled safety-bound adjustment when needed."><div className="grid gap-5 lg:grid-cols-2"><ContributionList title="Direct effects" items={result.directContributions} /><ContributionList title="Interaction effects" items={result.interactionContributions} /></div><div className="scroll-slim mt-5 overflow-x-auto"><table className="w-full min-w-[720px] border-collapse text-xs"><thead><tr className="border-b border-[var(--line)] text-left text-[9px] uppercase tracking-wider text-[var(--ink-faint)]"><th className="p-2">Indicator</th><th className="p-2 text-right">Baseline</th><th className="p-2 text-right">Direct</th><th className="p-2 text-right">Interaction</th><th className="p-2 text-right">Safety bound</th><th className="p-2 text-right">Final total</th></tr></thead><tbody>{(Object.keys(result.indicators) as IndicatorKey[]).map((key) => <tr key={key} className="border-b border-[var(--line)] last:border-0"><td className="p-2 font-bold">{indicatorLabels[key]}</td><td className="p-2 text-right font-mono">{format(result.baseline[key])}</td><td className="p-2 text-right font-mono">{signed(totals(key, result.directContributions))}</td><td className="p-2 text-right font-mono text-[var(--amber)]">{signed(totals(key, result.interactionContributions))}</td><td className="p-2 text-right font-mono text-[var(--red)]">{signed(result.clampAdjustments[key] ?? 0)}</td><td className="p-2 text-right font-mono font-bold text-[var(--accent)]">{format(result.indicators[key])}</td></tr>)}</tbody></table></div></ChartPanel>;
}
